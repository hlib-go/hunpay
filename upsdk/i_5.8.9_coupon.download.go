...
*/
func (up *Unionpay) CouponDownload(p *CouponDownloadParams) (r *CouponDownloadResult, err error) {
	bm := NewBodyMap()
	...
	signature, err := UpRsaSign(bm, up.Config.MchPrivateKey, false)
	if err != nil {
		return
	}
	...
	if p.Mobile != "" {
		// 注意：签名之后再加密敏感字段
		mobile, err := up.Encode3DES(p.Mobile)
		if err != nil {
			return nil, err
		}
	...
	if p.CardNo != "" {
		// 注意：签名之后再加密敏感字段
		cardNo, err := up.Encode3DES(p.CardNo)
		if err != nil {
			return nil, err
		}
	...
	resp, err := up.Post("/coupon.download", bm)
	if err != nil {
		return
	}
	switch resp.Resp {
