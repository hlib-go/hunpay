...
func (up *Unionpay) QualReduce(transNumber, activityNumber, qualNum, qualType, qualValue string) (r *QualReduceResult, err error) {
	bm := NewBodyMap()
	bm.Set("appId", up.Config.AppId)         // 是  接入方的唯一标识
	...
	signature, err := UpRsaSign(bm, up.Config.MchPrivateKey, true)
	if err != nil {
		return
	}
	...
	// 注意：签名之后再加密敏感字段
	qualValueEncode, err := up.Encode3DES(qualValue)
	if err != nil {
		return
	}
	bm.Set("qualValue", qualValueEncode)

	resp, err := up.Post("/qual.reduce", bm)
	if err != nil {
		return
	}
